...
function makeController({ inscripcionRepository } = {}) {
  ...
  async function eliminar(req, res) {
    try {
      ...
      // Solo estudiantes pueden eliminar su inscripción
      if (String((actor.rol || '')).toLowerCase() !== 'estudiante') {
        return res.status(403).json({ error: 'Solo estudiantes pueden eliminar su inscripción' });
      }
      ...
      const result = await repo.eliminarInscripcion(tutoriaId, estudianteId);

      if (result.status === 'not_found') return res.status(404).json({ error: result.message });
      if (result.status === 'not_enrolled') return res.status(400).json({ error: result.message });
      if (result.status === 'ok') return res.status(200).json({ message: result.message });

      // fallback genérico
      return res.status(500).json({ error: 'Resultado de eliminación inesperado' });
    } catch (err) {
      console.error('[inscripcionController.eliminar] error', err);
      return res.status(500).json({ error: 'Error interno al procesar la eliminación' });
    }
  }

  return { inscribir, eliminar };
}
