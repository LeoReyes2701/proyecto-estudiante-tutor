document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    // agregar credentials por si usas cookies para identificar usuario en el backend
    const [usuariosRes, tutoriasRes, inscripcionesRes] = await Promise.all([
      fetch('/usuarios', { credentials: 'same-origin' }),
      fetch('/tutorias', { credentials: 'same-origin' }),
      fetch('/inscripciones', { credentials: 'same-origin' })
    ]);

    if (!usuariosRes.ok || !tutoriasRes.ok || !inscripcionesRes.ok) {
      const errMsg = 'Error al cargar datos del servidor';
      console.error(errMsg, { usuariosResStatus: usuariosRes.status, tutoriasResStatus: tutoriasRes.status, inscripcionesResStatus: inscripcionesRes.status });
    ...
    // Mostrar todas las tutorías que el usuario NO está inscrito
    const disponibles = tutorias.filter(t => !tutoriasInscritas.includes(String(t.id)));

    renderTutorias(disponibles);
    ...
    tutorias.forEach(tutoria => {
      ...
      card.innerHTML = `
        <div class="card p-4 border rounded-3 shadow-sm h-100 d-flex flex-column justify-content-start align-items-center">
          <h3 class="fw-medium fs-4 mb-0">${tutoria.titulo || tutoria.nombre || 'Tutoria'}</h3>
  ...
  //  guardar selección en sessionStorage y redirigir a la página de horarios
  if (btnInscribirse) {
    btnInscribirse.addEventListener('click', async () => {
      ...
      // Guardar selección y redirigir a la página que manejará la selección de horarios
      try {
        const payload = { userId: currentUserId, tutoriaIds: seleccionadas };
        sessionStorage.setItem('selectedTutorias', JSON.stringify(payload));
        // Redirige a la página que creaste: horarios_seleccionados.html
        location.href = './horarioInscripcion.html';
      } catch (err) {
        console.error('Error guardando selección en sessionStorage', err);
        alert('No se pudo procesar la selección. Intenta de nuevo.');
      }
